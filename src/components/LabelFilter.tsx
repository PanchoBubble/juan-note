...
export function LabelFilter({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        isOpen &&
        !(event.target as Element).closest(".label-filter-dropdown")
      ) {
        setIsOpen(false);
      }
    };
        ...
        <div className="absolute top-full mt-1 w-64 bg-surface-secondary border-2 border-monokai-blue rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
          <div className="p-2">
            {availableLabels.length === 0 ? (
              <p className="text-sm text-monokai-comment text-center py-4">
                No labels available
              </p>
            ) : (
              <>
                ...
                {availableLabels.map(label => (
                  ...
                  >
                    <div
                      className={`w-3 h-3 rounded mr-2 border-2 ${getLabelColor(label)}`}
                    />
