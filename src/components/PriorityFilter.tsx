...
export function PriorityFilter({
  ...
  const priorities = [
    {
      value: null,
      label: "All",
      color: "text-monokai-comment bg-surface-tertiary border-monokai-yellow",
    },
    {
      value: 1,
      label: "Low",
      color: "text-monokai-yellow bg-surface-tertiary border-monokai-yellow",
    },
    {
      value: 2,
      label: "Medium",
      color: "text-monokai-orange bg-surface-tertiary border-monokai-yellow",
    },
    {
      value: 3,
      label: "High",
      color: "text-monokai-pink bg-surface-tertiary border-monokai-yellow",
    },
  ];

  const selectedPriorityItem = priorities.find(
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        isOpen &&
        !(event.target as Element).closest(".priority-filter-dropdown")
      ) {
        setIsOpen(false);
      }
    };
  ...
  return (
    <div className="relative priority-filter-dropdown">
      ...
      >
        <svg
          className="w-4 h-4 mr-2 text-monokai-comment"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <svg
          className={`w-4 h-4 ml-2 transition-transform text-monokai-comment ${isOpen ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
              ...
              <button
                ...
                className={`w-full text-left px-2 py-1 text-sm rounded mb-1 flex items-center text-monokai-fg hover:bg-surface-tertiary transition-colors ${
                  selectedPriority === priority.value
                    ? "bg-monokai-yellow bg-opacity-20 border border-monokai-yellow"
                    : ""
                }`}
              >
                <div
                  className={`w-3 h-3 rounded mr-2 border-2 ${priority.color}`}
                />
                <span className="flex-1">{priority.label}</span>
                {selectedPriority === priority.value && (
                  <svg
                    className="w-4 h-4 text-monokai-yellow"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
