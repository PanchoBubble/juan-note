...
export const NoteItemTitle = React.memo(function NoteItemTitle({ note }: NoteItemTitleProps) {
        ...
        <div className="mb-3">
            <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                    ...
                    {note.priority > 0 && (
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold border shadow-sm ${getPriorityColor(note.priority)} flex-shrink-0`}>
                            <span className="flex items-center space-x-1">
                                <span className={`w-1.5 h-1.5 rounded-full ${note.priority === 1 ? 'bg-monokai-yellow' :
